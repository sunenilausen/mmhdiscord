...
describe 'Map' do
  ...
  let!(:new_roberts) { LiveHost.create(map: roberts_map, bot_name: "mmh-4", message_id: "1234", channel_id: "1234") }
  ...
  let!(:new_wotj) { LiveHost.create(map: wotj_map, bot_name: "mmh-2", message_id: "1234", channel_id: "1234") }
  ...
  describe '.new_hosts' do
    it 'returns map that just went live when given a list of map hits' do
      expect(LiveHost.new_hosts(hits)).to eq(new_roberts)
      expect(LiveHost.new_hosts(hits)).to eq(new_wotj)
    end
  end
