...
describe 'Map' do
  ...
  describe '.new_hits' do
    it 'returns maps that is not live when given a list of map hits' do
      expect(LiveHost.new_hits(hits)).to include(roberts_hit)
      expect(LiveHost.new_hits(hits)).to include(wotj_hit)
      expect(LiveHost.new_hits(hits)).not_to include(bfme_hit)
    end
  end
